 ...
 */
@Controller
public class MaterialpayApi implements BaseVControlInf{
	...
	@ResponseBody
	@RequestMapping(value="materialpay",method=RequestMethod.POST,produces={"application/json;charset=UTF-8"})
	public Object materialpay(PayRequest payRequest){
		//验证
		List<ConstraintViolation> result = validator.validate(payRequest);
		if(result != null && result.size()>0){
			log.info("提交的数据有问题"+result.get(0).getMessage());
			return new BaseResponse(ResponseCode.DATAERR,result.get(0).getMessage());
		}
		return versionControl(payRequest.getApiversion(),payRequest);
	}
	...
	private Object version(Object object) {
		PayRequest payRequest=(PayRequest) object;
		return materialService.materialPay(payRequest);
	}
	
	private Object versionTwo(Object object) {
		PayRequest payRequest=(PayRequest) object;
		return materialService.materialPayV2(payRequest);
	}
