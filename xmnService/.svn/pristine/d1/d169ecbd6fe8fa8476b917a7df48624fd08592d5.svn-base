...
* @ClassName: ViewShipingAddressController    
* @Description:查看收货地址详情   
* @author: liuzhihao   
* @date: 2016年12月21日 下午5:29:49
 */
@Controller
@RequestMapping("/api/v1/market/pay")
public class ShipingAddressViewController implements BaseVControlInf{
	...
	@RequestMapping(value="/address/view",method=RequestMethod.POST,produces={"application/json;charset=UTF-8"})
	@ResponseBody
	...
	public Object versionOne(Object object){
		...
		map = shipingAddressService.findOneById(idRequest);
		if(!map.isEmpty()){
			response = new MapResponse(ResponseCode.SUCCESS,"查询成功");
		}else{
			response = new MapResponse(ResponseCode.SUCCESS,"查询失败");
		}
