...
<form id="listawary" action="${ctx}/h5/activity/kill/input" method="post">
<input type="hidden" name="secKillAmount" value="${kill.secKillAmount}"/>
	<input type="hidden" name="beginDate" value="<fmt:formatDate value='${kill.beginDate}' type="date" dateStyle="medium"/> "/>
	<input type="hidden" id="endDate" name="endDate" value="<fmt:formatDate value='${kill.endDate}' type="date" dateStyle="medium"/>" />
	<input type="hidden"
			name="beginTime"
			value="<fmt:formatDate value="${kill.beginTime}" type="both" pattern="HH:mm"/>" />
		<input type="hidden"
			name="endTime"
			value="<fmt:formatDate value="${kill.endTime}" type="both" pattern="HH:mm"/>" />
	<input type="hidden" name="name" value="${kill.name}"/>
	<input type="hidden" name="limitNumber" value="${kill.limitNumber}"/>
...
<script type="text/javascript">
	...
	$(function(){
		//对象数组转成js数组
		var arr=${kill!=null?kill.sellerCouponDetailsJson:""};
		if(arr!=null){
	...
	$("#createCoupon").on("click",function(){
		$("#listawary").attr("action","${ctx}/h5/activity/kill/input_award").submit();
	});
