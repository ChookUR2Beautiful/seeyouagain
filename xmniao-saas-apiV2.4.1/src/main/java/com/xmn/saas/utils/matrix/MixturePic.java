package com.xmn.saas.utils.matrix;
...
public class MixturePic {
	...
	public static void drawImage(BufferedImage bi, OutputStream ops) {
		InputStream is = null;
		try {
			// 1.jpg是你的 主图片的路径
//			File baseFile = new File(getUnderlayPicPath("smzf.jpg"));
			File baseFile = new File("c://base.jpg");
			BufferedImage buffImg = ImageIO.read(baseFile);
			// 得到画笔对象
			Graphics g = buffImg.getGraphics();

			// 将小图片绘到大图片上
			g.drawImage(bi, 477, 872, null);
			g.dispose();
...
//		System.out.println("fmt=" + fmt);
	...
	public static BufferedImage testDraw() {
		...
		try {
			...
			// 创建你要附加的图象。
			// 2.jpg是你的小图片的路径
			File sFile = new File("c://1.jpg");
			BufferedImage bi = ImageIO.read(sFile);

			// 创建新的图象
			 buffImg = new BufferedImage(base.getWidth(),
			...
			g.drawImage(bi, 1400, 3900, null);

			g.dispose();

			os = new FileOutputStream("c://finish.jpg");
			// 创键编码器，用于编码内存中的图象数据。
	...
	private static void drawImageUtil2(BufferedImage bi,
		...
		try {
			...
			// 定义文字的内容和大小（商家名字）
			Font font = new Font("微软雅黑", Font.BOLD, 78);
			//
			// 3.jpg是你的 主图片的路径
			File baseFile = new File(getUnderlayPicPath(path));
			BufferedImage buffImg = ImageIO.read(baseFile);
