...
public class RSARB {
	...
	String pub = new String(Base64RB.encodeBase64(publicKeyBytes), CHAR_ENCODING);
	/** 得到私钥 */
	Key privateKey = kp.getPrivate();
	byte[] privateKeyBytes = privateKey.getEncoded();
	String pri = new String(Base64RB.encodeBase64(privateKeyBytes), CHAR_ENCODING);
	...
	byte[] deBase64Value = Base64RB.encodeBase64(b);
	...
	return new String(Base64RB.encodeBase64(b1), CHAR_ENCODING);
    }
	...
	byte[] b1 = Base64RB.decodeBase64(cryptograph.getBytes());
	/** 执行解密操作 */
	...
	X509EncodedKeySpec keySpec = new X509EncodedKeySpec(Base64RB.decodeBase64(key.getBytes()));
	...
	PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(Base64RB.decodeBase64(key.getBytes()));
	KeyFactory keyFactory = KeyFactory.getInstance("RSA");
	...
	try {
	    PKCS8EncodedKeySpec priPKCS8 = new PKCS8EncodedKeySpec(Base64RB.decodeBase64(privateKey.getBytes()));
	    ...
	    return new String(Base64RB.encodeBase64(signed));
	} catch (Exception e) {
	...
	try {
	    KeyFactory keyFactory = KeyFactory.getInstance("RSA");
	    byte[] encodedKey = Base64RB.decode2(publicKey);
	    ...
	    boolean bverify = signature.verify(Base64RB.decode2(sign));
	    return bverify;
