...
public class WalletConsumer {
	...
	@Autowired
	private LiveWalletAccess liveWalletAccess;
...
//					Map<String, String> paramMap = JSON.parseObject(message,new TypeReference<Map<String, String>>() {});
					...
					try {
						...
						//同时更新会员钱包和直播钱包
							...
							recordid = paramList.get(0).get("recordid");
							userType = paramList.get(0).get("userType");
						}else if(msg.getTags().equals("live_gift_ledger")){
							//打赏主播礼物,添加鸟蛋 & V客推荐分账钱包 
							Map<String, String> param = JSON.parseObject(message,new TypeReference<Map<String,String>>(){});
							Boolean bool;
							try {
								bool=liveWalletAccess.liveGiftLedger(param);
							} catch (Exception e) {
								log.error("更新失败",e);
								bool = false;
							}
