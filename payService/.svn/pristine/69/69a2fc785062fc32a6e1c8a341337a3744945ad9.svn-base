...
//            }
    ...
    public Result sendPropsRedpackage(String transNo, long uid, List<Long> giveUids, int number, double amount, int type,String phone) throws TException {
        Result result = new Result();
        try {
            result = propsService.sendPropsRedpackage(transNo, uid, giveUids, number, amount, type,phone);
        } catch (CustomException e) {
            result.setCode(ResultCodeEnum.ERROR.status());
            result.setStatusCode(BusinessStatusCodeEnum.FAIL_ERROR_STATUS.getStatus());
     ...
     */
    @Override
    public Result exchangeMoneyRedpackage(String transNo, long uid, int number,  double moneyRedpackage) throws TException {
        Result result = new Result();
        try {
            result = propsService.exchangeMoneyRedpackage(transNo, uid, number, moneyRedpackage);
        } catch (CustomException e) {
    ...
    public ResultList listAllNotUseGiveFlower(long uid) {
        ResultList result = new ResultList();
        try {
            result = propsService.listAllNotUseGiveFlower(uid);
        } catch (Exception e) {
            logger.error("获取用户所有未使用的下级赠送的花苗，异常信息如下:" + e);
            result.setCode(ResultCodeEnum.ERROR.status());
            result.setExceptionMessage(e.getMessage());
            result.setStatusCode(BusinessStatusCodeEnum.SYSTEM_ERROR_STATUS.getStatus());
            result.setMessage("获取用户所有未使用的下级赠送的花苗，异常信息如下:" + e);
        }
        return result;
    }
    ...
    public Result growGiveFlower(String transNo,long uid ,List<Long> giveUids) {
        Result result = new Result();
        try {
            result = propsService.growGiveFlower(transNo, uid, giveUids);
        } catch (CustomException e) {
            logger.error("当前用户" + uid + "使用园又赠送的花苗进行种花，异常信息如下:" + e);
            result.setCode(ResultCodeEnum.ERROR.status());
            result.setExceptionMessage(e.getMessage());
            result.setStatusCode(BusinessStatusCodeEnum.FAIL_ERROR_STATUS.getStatus());
            result.setMessage("当前用户" + uid + "使用园又赠送的花苗进行种花，异常信息如下:" + e);
        } catch (Exception e) {
            logger.error("当前用户" + uid + "使用园又赠送的花苗进行种花，异常信息如下:" + e);
            result.setCode(ResultCodeEnum.ERROR.status());
            result.setExceptionMessage(e.getMessage());
