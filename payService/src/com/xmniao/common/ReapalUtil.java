...
public class ReapalUtil {
	...
	// 商户获取融宝公钥
	PublicKey pubKeyFromCrt = RSARB.getPubKeyFromCRT(pubKeyUrl);
	// 随机生成16数字
	String key = RandomUtil.getRandom(16);

	System.out.println("key=============>" + key);
	// 对随机数进行加密
	String encryptkey = RSARB.encrypt(key, pubKeyFromCrt);
	...
	// 获取自己私钥解密
	PrivateKey pvkformPfx = RSARB.getPvkformPfx(privateKey, ReapalUtil.getPassword());
	String decryptData = RSARB.decrypt(encryptkey, pvkformPfx);
