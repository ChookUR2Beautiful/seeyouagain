...
public class AutoClassifyBalanceServiceImpl3 implements AutoClassifyBalanceService{

	 //初始日志类
	private final Logger log = Logger.getLogger(AutoClassifyBalanceServiceImpl3.class);
	...
	public void autoClassifyBalance() {
		...
		//1.建表3个
		String curDate = "150412";//new SimpleDateFormat("yyMMdd").format(new Date());
		final String curUserTable = USER_TABLE+curDate.substring(0, 4);
		final String curSellerTable = SELLER_TABLE+curDate.substring(0, 4);
		final String curJonitTable = JOINT_TABLE+curDate.substring(0, 4);
		
		new Thread(curUserTable){
			@Override
			public void run() {
				createUserTable(curUserTable,USER_TYPE);
				saveWalletInfo(USER_TYPE,curUserTable);
				log.info("-------插入用户表数据成功------");
				super.run();
			}
		}.start();
		new Thread(curSellerTable){
			@Override
			public void run() {
				createUserTable(curSellerTable,SELLER_TYPE);
				saveWalletInfo(SELLER_TYPE,curSellerTable);
				log.info("-------插入商户表数据成功------");
				super.run();
			}
		}.start();
		new Thread(curJonitTable){
			@Override
			public void run() {
				createUserTable(curJonitTable,JOINT_TYPE);
				saveWalletInfo(JOINT_TYPE,curJonitTable);
				log.info("-------插入合作商表数据成功------");
				super.run();
			}
		}.start();
