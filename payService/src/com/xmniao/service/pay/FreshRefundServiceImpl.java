 ...
 *
 */
@Service("FreshRefundServiceImpl")
public class FreshRefundServiceImpl implements FreshRefundService.Iface{
	
	 //初始日志类
	private final Logger log = Logger.getLogger(FreshRefundServiceImpl.class);
	...
	public Map<String, String> FreshRefund(RefundRequest refundRequest)
		...
		log.info("FreshRefund----->refundRequest.orderNumer:"+refundRequest.getOrderNumber()+"----->refundRequest.money:"
		...
		try{	
			...
			refundService.updateFreshBusineStatus(refundRequest.getOrderNumber(), resultMap.get("code"), resultMap.get("Msg"));
			//1.查询该支付订单信息
			...
			try{
				...
				resultMap = returnMap(StateCodeUtil.PR_PARAM_ERROR,"订单号格式错误");
				return resultMap;
			}
			
			Map<String,Object> payOrder = FreshOrderMapper.getPayOrderByorderNumber(orderNumber);
			if(payOrder == null){
				log.error("订单号:"+orderNumber+",未找到该订单的支付记录");
				resultMap = returnMap(StateCodeUtil.PR_ORDER_NOT_EXIST,"未找到该订单的支付记录");
				return resultMap;
			}
			log.info("查询到的订单信息：\r\n"+JSON.toJSONString(payOrder));
			
			//2.验证该支付订单是否符合退款需求
			boolean isChecked = checkOrderStatus(payOrder,refundRequest,resultMap);
			if(!isChecked){
			...
			PayRefund payRefund = refundService.getFreshRefundRecordBypayId((long)payOrder.get("payId"));
			if(payRefund != null){
				
				//验证退款记录表中，退款记录状态
				if(payRefund.getStatus() == PayRefundStatus.PROCESS_SUCCESS ){	//已退款
					refundService.updateFreshBusineStatus(refundRequest.getOrderNumber(),StateCodeUtil.PR_SUCCESS,"退款成功!");
					
					log.info("订单号:"+orderNumber+",该订单已退款");
					resultMap = returnMap(StateCodeUtil.PR_REFUNDED,"退款成功!");
					return resultMap;
				}
				...
				if(payRefund.getStatus() == PayRefundStatus.PROCESSING){		//正在处理中，
					if(thirdName.compareToIgnoreCase(PayTypeUtil.alipay) == 0){
						...
						resultMap = returnMap(StateCodeUtil.PR_REQUEST_REPEAT,"已成功申请退款");
						return resultMap;
					}
				}
				...
				//将退款记录状态更新为"正在处理中"
				refundService.updateRefundStatus(payRefund.getRefundId(), PayRefundStatus.PROCESSING,1);
				...
				payRefund = refundService.addFreshRefundRecord(payOrder);
				if(payRefund == null){
					log.error("订单号:"+orderNumber+",添加该订单的退款记录异常");
					resultMap = returnMap(StateCodeUtil.PR_SYSTEM_ERROR,"添加退款记录异常");
					return resultMap;
				}
...
//						return resultMap;
			...
			if(!bSuccess){
				...
				resultMap = refundService.refundSubmit(payRefund,refundRequest.getOrderNumber(),1);
			...
			if(RefundUtil.isRefundSuccess(resultMap)){
				refundService.updateRefundStatus(payRefund.getRefundId(),PayRefundStatus.PROCESS_SUCCESS,1);
			}
			else if(RefundUtil.isRefundFail(resultMap)){
				refundService.updateRefundStatus(payRefund.getRefundId(),PayRefundStatus.PROCESS_Failure,1);
			}
		
			//5.调用业务退款接口 退款处理中,状态不需更新
//			if(!RefundUtil.isRefunding(resultMap)){
//				Map<String, String> busineMap =refundService.updateFreshBusineStatus(refundRequest.getOrderNumber(),resultMap.get("code"),resultMap.get("Msg"));
//				if(busineMap == null){
...
//			}
	 ...
	 * @return
	 */
	public boolean checkOrderStatus(Map<String,Object> payOrder,RefundRequest refundRequest,Map<String,String> resultMap){
		//1.查询到对应的支付订单
		if(payOrder == null || payOrder.get("payId")==null){
			log.error("未找到该订单的支付记录或在支付记录中支付ID为空");
			resultMap = returnMap(StateCodeUtil.PR_ORDER_NOT_EXIST,"未找到该订单的支付记录或在支付记录中支付ID为空");
			return false;
		}
...
//			resultMap = returnMap(StateCodeUtil.PR_ORDER_DISENABLE,"该支付订单类型不允许退款");
//			return false;
//		}
...
//			resultMap = returnMap(StateCodeUtil.PR_ORDER_NOTPAY,"该支付订单未完成支付，不允许退款");
//			return false;		
//		}
...
//			resultMap = returnMap(StateCodeUtil.PR_MONEY_ERROR,"退款金额错误");
//			return false;		
//		}
		...
		if(thirdName.equals(PayTypeUtil.umpay)){
			...
			if(today.equals(thirdPayDate)){
				...
				resultMap = returnMap(StateCodeUtil.PR_ORDER_DISENABLE,"U付订单：当天支付订单不可退款");
				return false;
			}
		}
...
//		return result;
	 ...
	 * @return
	 */
	public Map<String, String> updateBusineStatusOld(String bId,String code,String msg){
