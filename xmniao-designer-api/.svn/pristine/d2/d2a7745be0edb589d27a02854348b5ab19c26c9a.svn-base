...
<input type="hidden" value="${type}" id="type"/>
    ...
    <div class="floor-module">
        <a class="floor-links" href="${pageContext.request.contextPath}/h5/coupon/input?couponType=${type}">创建赠品券</a>
    </div>
</body>
...
$(function(){
    ...
    function detail(id){
		window.location="${pageContext.request.contextPath}/h5/coupon/detail?type=7&couponId="+id;
	}
    //加载列表数据
    $(function(){
    	var couponType = $("#type").val();
    	//已结束
    	$.get("${pageContext.request.contextPath}/h5/coupon/list?status=0&type=4",function(data){
    		if(data.state==0){
    			...
    			if(data.result==undefined){
    				...
    				$.each(data.result, function(i, v) {
    	    			...
    	    			"<i class='icon-wrap icon-zeng-2'></i>",v.cname,"</span></div>",
    	                ...
    	                "<div class='item-floor col-odd-1'><span class='floor-time'><em class='time-date'>活动时间：",v.startTime,"至",v.endTime,"</em><div class='more-links'><i></i><i></i><i></i></div>",
    	                "</span></div></a>",
    	                "</div>"].join("");
    	    			$("#div-end").append(item);
    	    			});
    	...
    	//进行中
    	$.get("${pageContext.request.contextPath}/h5/coupon/list?status=1&type=4",function(data){
    		if(data.state==0){
        			if(data.result==undefined){
        				var str="<div class='emptydata'>";
        				str+="<i></i>";
        				...
        				$.each(data.result, function(i, v) {
        	    			var item = ["<div class='slide-item'>",
        	    			"<a class='item-links' href='#' onclick='detail("+v.id+")'><div class='item-head'><span class='tit-desc'>",
        	    			"<i class='icon-wrap icon-zeng-3'></i>",v.cname,"</span></div>",
        	                "<div class='item-content'><span class='content-size'><b>",v.sendNum,"</b><em>领取个数</em></span><span class='content-time'><b>",v.useNum,"</b><em>使用个数</em></span></div>",
        	                "<div class='item-floor'><span class='floor-limit'><em class='limit-total'>生成总数：",v.maximum,"</em>",
        	                "<em class='limit-sum'>剩余数量：",(v.maximum-v.sendNum),"</em></span><span class='floor-time'>",
        	                "<em class='time-date'>",v.startTime,"至",v.endTime,"</em><div class='more-links'><i></i><i></i><i></i></div>",
        	                "</span></div></a><a class='icon-wrap share-links' href='javascript:;' id='"+v.id+"' value='"+v.cname+"' onclick='share(this,7)'></a>",
        	                "</div>"].join("");
