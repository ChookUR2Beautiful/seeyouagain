...
public class OrderPayServiceImpl implements Iface {
	...
	public ResponseData updateOrderInfo(Map<String, String> walletMap) throws FailureException, TException {
		logger.info("[调用支付接口回调]order_no="+walletMap.get("order_no"));
		try {
			...
			order1.setStatus(DesignerConsts.ORDER_STATUS_IS_PAYMENT);
			order1.setVersionLock(order.getVersionLock());
			order1.setOutTradeNo(payId);
			...
			if(DesignerConsts.ORDER_TYPE_MATERIAL.equals(order.getType())){
				OrderMaterial orderMaterial = orderMaterialDao.selectByOrderNum(orderNo);
				Material material = materialDao.selectByPrimaryKey(orderMaterial.getMaterialId());
				Date paytime = material.getLastPaytime();
				Object key=new Object();
				if(paytime==null||paytime.getMonth()!=new Date().getMonth()){
					key=null;
				}
				materialDao.updateSoldTimes(orderMaterial.getMaterialId(),key);
			}
			return new ResponseData(ResponseCode.SUCCESS,"更新支付状态成功",null);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("[调用支付回调出现异常]order_no="+walletMap.get("order_no"));
