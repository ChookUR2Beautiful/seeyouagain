package com.xmn.designer.service.base;
...
public class RedisService {
    ...
    public static final String PREFIX_DESIGNER_ACCOUNT = "saas:designer:account:";
    public static final String PREFIX_DESIGNER_ACCOUNT_AID = "saas:designer:account:id:";
    public static final String PREFIX_DESIGNER_PAY_COUNT = "saas:designer:api:pay:count:";
    /**
     * 初始日志类
     */
    ...
    /**
     * 将外部登陆自设计需要缓存的对象保存在Redis中,默认缓存一个月 并返回sessionToken
     * @param redisKey
     * @param sellerAccount
     */
    public String setDesignerSessionCache(User user) {
        String aidRedisKey = PREFIX_DESIGNER_ACCOUNT_AID + user.getId();
        if(this.exsitKey(aidRedisKey)) {
            // 通过aid获取已登录的sessionToken
            String oldSessionToken = this.getString(aidRedisKey);
            // 删除已登录的sessoinToken
            this.deleteKey(oldSessionToken);
        }
        ...
        // 将登录缓存对象以Hash数据格式,存入Redis中,
        // 存入Redis, redis key 需要加上前缀
        this.setHash(PREFIX_DESIGNER_ACCOUNT+sessionToken, cacheObjectMap, calendar.getTime());
        this.setString(aidRedisKey, PREFIX_DESIGNER_ACCOUNT+sessionToken, calendar.getTime());
     ...
     * @return
     */
    public boolean checkDesignerSessionCacheObject(String sessionToken) {
        sessionToken = PREFIX_DESIGNER_ACCOUNT + sessionToken;
        return redisTemplate.hasKey(sessionToken);
    }
    
    
   /**
    * 根据sessionToken获取redis中User的信息
    * @param sessionToken
    * @return
    */
    public User getDesignerUser(String sessionToken) {
        String className = User.class.getSimpleName(); 
        // 根据ressionToken取出Redis中缓存的Hash数据
        Map<Object, Object> cacheObjectMap = this.getHash(PREFIX_DESIGNER_ACCOUNT + sessionToken);
