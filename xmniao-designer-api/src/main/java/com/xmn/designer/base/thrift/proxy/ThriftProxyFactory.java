package com.xmn.designer.base.thrift.proxy;
	...
	public void run() {
	    ...
	    if (null != proxys && !proxys.isEmpty())
	    {
	        try
	        {
	            ...
	            {
	                //获取接口类中的Iface接口
	                Class<?> Processor = Class.forName(serverProxy.getServiceInterface() + "$Processor");
	                Class<?> Iface = Class.forName(serverProxy.getServiceInterface() + "$Iface");
	                Constructor<?> con = Processor.getConstructor(Iface);
	                //获取接口实现类
	                tProcessor = (TProcessor)con.newInstance(serverProxy.getServiceImpl());
	            ...
	            log.info("业务系统服务已启动...  工作线程数:"+workerThreads+" 端口:"+port);
	            server.serve();
	        }
