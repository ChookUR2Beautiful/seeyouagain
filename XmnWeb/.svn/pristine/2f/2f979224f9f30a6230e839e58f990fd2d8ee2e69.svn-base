...
/**
 * 来源身份初始化,来源身份  1:普通兼职  2:工会合作 3:活动合作 4:其他 5:区域（0：对应sign_type为签约或公司账号）
 */
function rootRoleInit(){
//	debugger;
	var rootRole=$("#rootRole").val();
	if(rootRole==4){
		$("#remarkDiv").css("display","inline");
	}else {
	...
	if(rootRole==5){
		...
		$("select[name='province'] option:first").prop("selected", 'selected');  
		$("select[name='city'] option:first").prop("selected", 'selected');  
