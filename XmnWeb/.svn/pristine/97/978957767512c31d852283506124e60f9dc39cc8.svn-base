...
+ function($, window)
{
    ...
    DataTable.prototype.render = function()
    {
        ...
        if (data.flexArea)
        {
        	...
        	if(typeof rowColLen === 'undefined' || rowColLen===0){
        		var t = $('<td>').attr(
                        {
                            "data-row"   : 0,
                            "data-index" : 0,
        		...
        		$flex.append($('<tr/>').append(t));
        	}
            $rowSpan = $(dataRowSpan);
            $rowSpan.addClass('flexarea').find('.datatable-wrapper').attr("style","overflow: auto; max-height: 400px; width: 100%")
                    ...
                    .append('<div class="scrolled-shadow scrolled-in-shadow"></div><div class="scrolled-shadow scrolled-out-shadow"></div></div>')
                    .find('table')
                    .addClass(options.tableClass)
                    .append($flex);
            $rows.append($rowSpan);
    ...
    DataTable.prototype.bindEvents = function()
    {
        ...
        if(data.flexArea)
        {
            ...
            $scrollbar.mousedown(function(event)
            {
        	 ...
        	 var $headTable=$headSpans.find('.table'),$tableWrapper = $rowsSpans.find('.datatable-wrapper');
        	 $tableWrapper.on( 'scroll', function (e) {
        		 $headTable.css("left","-"+this.scrollLeft+"px");
			});
        }
