...
@Controller
@RequestMapping(value="user_terminal/userProposal")
public class TUserProposalController extends BaseController {
	@Autowired
	...
	public String init(){
		return "user_terminal/UserProposalList";
	}
	/**
	 * 
	 * @author dong'jt
	...
	public ModelAndView updateStatusInit(){
		ModelAndView modelAndView=new ModelAndView("user_terminal/updateUserProposal");
		return modelAndView;
	}
	/**
	 * 
	...
	public Object updateStatus(TUserProposal tUserProposal,HttpServletRequest request){
		...
		try {
			...
			this.log.error("用户吐槽处理："+e);
			tUserProposalService.fireLoginEvent(StringUtils.addStrToStrArray(((ApplicationException)e).getLogInfo(),new ApplicationException("用户吐槽处理", e, new Object[]{tUserProposal,request}).getMessage()),0);
		}
		return resultable;	
	}
}
