...
<body>
	<form id="editFrom" role="form" class="form-horizontal">
		<c:if test="${!empty liverInfo}">
			<input type="hidden" name="id" id="id" value="${liverInfo.id}">
		</c:if>
		<input type="hidden" id="bindSuperiorInfoToken" name="bindSuperiorInfoToken" value="${bindSuperiorInfoToken }">
		<input type="hidden" name="uid" id="uid" value="${liverInfo.uid}">
		<!-- 会员平台 1 寻蜜鸟 2 中脉 -->
		<input type="hidden" name="userPlatform" id="userPlatform" value="${liverInfo.userPlatform}">
		<!-- 会员类型 1 VIP 2 商家 3 直销 -->
		<input type="hidden" name="objectOriented" id="objectOriented" value="${liverInfo.objectOriented}">
		<!-- 上级Uid -->
		<input type="hidden" name="superiorUid" id="superiorUid" value="${liverInfo.superiorUid}">
		...
		<div class="form-group" id="indirectUidDiv">
	...
	<script src="<%=path%>/js/live_anchor/bindSuperiorInfo.js?v=1.0.11"></script>
</body>
</html>
