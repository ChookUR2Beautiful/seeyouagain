...
$(document).ready(function() {
	appPushList = $('#appPushList').page({
		url : 'business_cooperation/appPush/init/list.jhtml',
		success : success,
		pageBtnNum : 10,
	...
	inserTitle(' > 合作商管理 > <a href="business_cooperation/appPush/init.jhtml" target="right">推送消息管理</a>','business_cooperation_appPush',true);

	$("#export").click(function(){
		$form = $("#searchForm").attr("action","business_cooperation/appPush/export.jhtml");
		$form[0].submit();
	});
});
...
*/				html.push('<td>');
				if(permissions.update=='true'){
					html.push('<a href="javascript:viod(); " data-type="ajax"   data-url="business_cooperation/appPush/update/init.jhtml?tid='+data.content[i].tid+'"  data-toggle="modal" >查看</a>&nbsp;&nbsp;');
				}
...
*/				}
			...
			html.push('<td>' + (undefined == data.content[i].object ? "全部" : data.content[i].object) + '</td>');
...
function remove(tid) {
	...
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'business_cooperation/appPush/delete.jhtml' + '?t=' + Math.random(),
			data : 'tid=' + tid,
			dataType : 'json',
