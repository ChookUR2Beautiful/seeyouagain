$.fn.searchChosen2 = function(param){
	var $this = $(this);
	...
	var build = function(){
		...
		html.push('<ul class="chosen-choices chosen-2"></ul>');
		html.push('</div>');	
		html.push('<button type="button" class="btn btn-primary" id="searchChosen2" style="float:left;margin-left: 10px;">请选择</button>');
		...
		//生成选择页面
		$("#searchChosen2").click(function(){
			var choicesHtml = $choices.html();
			//初始化选择页面模态框
			$("#choseModal2").remove();//处理刷新问题
			if(!$("#choseModal2").length){
				$("body").append('<div class="modal fade" id="choseModal2" aria-hidden="true" data-position="10px" style="display: none;z-index: 9990;"><div class="modal-dialog modal-lg" style="margin-top: 151.33333333333334px;width:1400px;"><div class="modal-content"><div class="modal-header"><button type="button" class="close"  data-dismiss="modal"><span class="sr-only">关闭</span></button><h4 class="modal-title">请选择</h4></div><div class="modal-body"><iframe id="iframetriggerModal1" src="'+opts.url+'" frameborder="no"allowtransparency="true" scrolling="auto"style="width: 100%; height: 100%; left: 0px;"></iframe></div></div></div></div>');
				$("#choseModal2").find("iframe").load(function(){
					...
					}).find(".chosen-2").html(choicesHtml);//初始化值
					$(wdocument).find(".closeChosen").click(function(){
						$("#choseModal2").modal('hide');
					});
				});
				$("#choseModal2").modal();
			}else{
				var iframe=$("#choseModal2").find("iframe")[0];
				$(iframe.contentWindow.document).find(".chosen-2").html(choicesHtml);
		...
		//删除事件
		$div.find("ul.chosen-2").on("click", "a.search-choice-close", function(){
			$(this).parent().remove();
			//$this.val($this.val().replace($(this).attr("data")+ opts.separator, ""));
			//添加的时候调用了encodeURI，删除的时候也要调用,
			$this.val($this.val().replace(encodeURI($(this).attr("data"))+opts.separator,""));
			if(document.getElementById('iframetriggerModal1')){
				$(document.getElementById('iframetriggerModal1').contentWindow.document).find("a[data='"+$(this).attr("data")+"']").parent().remove();
			}
		});
	}
	...
	var showChose = function(value, show){
		...
		$(document.getElementById('iframetriggerModal1').contentWindow.document).find("#choseDatas").find(".chosen-2").append($(createLi(value,show)));
	}
	...
	var chose = function(value, show,obj){
		if(obj.checked){
			...
			if(document.getElementById('iframetriggerModal1')){
				$(document.getElementById('iframetriggerModal1').contentWindow.document).find("a[data='"+value+"']").parent().remove();
			}	
		}
		var $subBox = $(wdocument).find("input[items='oneCheck']");
        $(wdocument).find("input[id='allCheck']").attr("checked",$subBox.length == $(wdocument).find("input[items='oneCheck']:checked").length ? true : false);
	...
	var memberChose = function(value, show,obj){
		if(obj.checked){
			...
			if(document.getElementById('iframetriggerModal1')){
				$(document.getElementById('iframetriggerModal1').contentWindow.document).find("a[idData='"+value+"']").parent().remove();
			}
		}
	}
	//会员显示已选择项
	var memberShowChose = function(value,show,datas){
		...
		$(document.getElementById('iframetriggerModal1').contentWindow.document).find("#choseDatas").find(".chosen-2").append('<li class="search-choice"><span>'+show+'</span><a class="search-choice-close" idData="'+value+'" data="'+datas+'"></a></li>');
	}
	/*//显示已选择项
	var memberShowChose = function(value, show){
		...
		$(document.getElementById('iframetriggerModal1').contentWindow.document).find("#choseDatas").find(".chosen-2").append('<li class="search-choice"><span>'+show+'</span><a class="search-choice-close" data="'+value+'"></a></li>');
	}
    ...
    var allCancel = function(){//清除已选择内容的方法   dongjt
			...
			if(document.getElementById('iframetriggerModal1')){
			  $(document.getElementById('iframetriggerModal1').contentWindow.document).find(".search-choice").remove();//取消弹出页面选择内容
			}	
    }
	$.extend($this,{
