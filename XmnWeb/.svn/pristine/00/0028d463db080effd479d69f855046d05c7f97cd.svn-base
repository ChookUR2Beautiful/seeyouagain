...
//		addBannerInfo(liveRecord);
	 ...
	 * 方法描述：加载通告机器人配置信息 <br/>
	 * 创建人：  huang'tao <br/>
	 * 创建时间：2017-7-24上午11:03:34 <br/>
	 * @param liveRecord
	 */
	public void loadRobotSetInfo(TLiveRecord liveRecord) {
		Integer recordId = liveRecord.getId();
		TLiveRobotSet robotSetReq=new TLiveRobotSet();
		robotSetReq.setRecordId(recordId);
		TLiveRobotSet liveRobotSet = liveRobotSetService.selectByBean(robotSetReq);
		if(liveRobotSet != null){
			liveRecord.setRobotSetMixNums(liveRobotSet.getRobotSetMixNums());
			liveRecord.setRobotSetMaxNums(liveRobotSet.getRobotSetMaxNums());
			liveRecord.setRobotMinNums(liveRobotSet.getRobotMinNums());
			liveRecord.setRobotMaxNums(liveRobotSet.getRobotMaxNums());
			liveRecord.setMultiple(liveRobotSet.getMultiple());
		}
	 ...
	 * 方法描述：同步通告机器人设置信息 <br/>
	 * 创建人：  huang'tao <br/>
	 * 创建时间：2017-7-24上午11:37:41 <br/>
	 * @param liveRecord
	 */
	public void syncRobotSetInfo(TLiveRecord liveRecord) {
		// TODO Auto-generated method stub
		...
		try {
			Integer recordId = liveRecord.getId();
			if(recordId!=null){
				liveRobotSetService.deleteByRecordId(recordId);
				Boolean hasKey = redisTemplate.hasKey(LiveConstant.LIVE_ROBOT_SET_KEY+recordId);
				if(hasKey){
					redisTemplate.delete(LiveConstant.LIVE_ROBOT_SET_KEY+recordId);
				}
				
				
				Integer isRobotInit = liveRecord.getIsRobotInit();
				if(isRobotInit!=null && isRobotInit.intValue()==1){
					Integer robotSetMixNums = liveRecord.getRobotSetMixNums();//单次最少新增机器人个数
					Integer robotSetMaxNums = liveRecord.getRobotSetMaxNums();//单次最多新增机器人个数
					Integer robotMinNums = liveRecord.getRobotMinNums();//初始化机器人数量
					Integer robotMaxNums = liveRecord.getRobotMaxNums();//最高上限机器人数量
					Integer multiple = liveRecord.getMultiple();//显示倍数
					...
					TLiveRobotSet liveRobotSet = new TLiveRobotSet();
					liveRobotSet.setRobotSetMixNums(robotSetMixNums);
					liveRobotSet.setRobotSetMaxNums(robotSetMaxNums);
					liveRobotSet.setRobotMinNums(robotMinNums);
					liveRobotSet.setRobotMaxNums(robotMaxNums);
					liveRobotSet.setMultiple(multiple);
					liveRobotSet.setStatus(1);
					liveRobotSet.setRecordId(recordId);
					liveRobotSet.setIsCommon(0);
					liveRobotSet.setCreateTime(new Date());
					liveRobotSet.setUpdateTime(new Date());
					
					liveRobotSetService.add(liveRobotSet);
					
					redisTemplate.opsForHash().putAll(LiveConstant.LIVE_ROBOT_SET_KEY+recordId, MapBeanUtil.convertMap(liveRobotSet, "robotSetMixNums","robotSetMaxNums","robotMinNums","robotMaxNums","multiple"));
			...
			e.printStackTrace();
			this.log.error("执行同步通告机器人设置信息方法——>syncRobotSetInfo()异常:"+e.getMessage(), e);
		}
