	...
	<form class="form-horizontal" role="form" id="editActivityForm">
		...
		<table width="100%">
			<tbody>
				...
				<tr>
					<th style="width:90px;"><h5>&nbsp;&nbsp;佣金补贴(%)：</h5></th>
					<td colspan="2"><input type="text" class="form-control"
...
/* 	
	...
	//	var istype = checkType(data.type,data.rule);
		if($('#isType').val() ==  'add'){
			...
			url = 'marketingManagement/activityManagement/commission/add.jhtml' + '?t=' + Math.random();
			...
			//if(istype){
				...
				url = 'marketingManagement/activityManagement/commission/update.jhtml' + '?t=' + Math.random();
				$.ajax({
					type : 'post',
					url : url,
					data : data,
