...
<body>
	...
	<div id="businessSignedList"></div>
	<script type="text/javascript">
		$(document).ready(function() {
		var businessSignedList; 
		businessSignedList = $('#businessSignedList').page({
				url : 'business_area/businessArea/init/getbusinessSignedInfo/init/list.jhtml',
				success : success,
				pageBtnNum : 7,
	...
	function success(data, obj) {
		...
		html.push('<th style="width:50%;">已签约商家</th>');
		html.push('</tr>');
		html.push('</thead>');
		html.push('<tbody');
		...
		for (var i = 0; i < data.content.length; i++) {		
			...
			html.push('<td>' + (undefined == data.content[i].business ? "-" : data.content[i].business) + '</td>');
			html.push('</tr>');
		}
