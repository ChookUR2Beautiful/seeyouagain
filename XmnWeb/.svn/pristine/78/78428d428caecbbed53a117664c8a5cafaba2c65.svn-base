...
$(document).ready(function() {
	systemInfoList = $('#systemInfoList').page({
		url : 'common/systemInfo/init/list.jhtml',
		success : success,
		pageBtnNum : 10,
	...
	inserTitle(' > 合作商管理 > <a href="common/systemInfo/init.jhtml" target="right">系统信息发布管理</a>','userSpan',true);

	$("#export").click(function(){
		$form = $("#searchForm").attr("action","common/systemInfo/export.jhtml");
		$form[0].submit();
	});
...
function success(data, obj) {
	...
	for (var i = 0; i < data.content.length; i++) {
		...
		html.push('<td><a href="javascript:viod(); " data-type="ajax"   data-url="common/systemInfo/update/init.jhtml?id='+data.content[i].id+'"  data-toggle="modal" >修改</a>&nbsp;/&nbsp;<a href="javascript:remove('+data.content[i].id+')">删除</a></td>');
...
function remove(id) {
	...
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'common/systemInfo/delete.jhtml' + '?t=' + Math.random(),
			data : 'id=' + id,
			dataType : 'json',
