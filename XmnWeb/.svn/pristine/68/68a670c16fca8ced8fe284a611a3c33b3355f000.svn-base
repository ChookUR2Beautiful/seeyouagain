...
$(document).ready(function() {
	cardList = $('#cardList').page({
		url : 'anchor/manage/bindCard/bindCardList.jhtml',
		success : success,
		pageBtnNum : 10,
	...
	inserTitle('> <a href="businessman/multipShop/bindCardInit.jhtml?sellerid='+sellerid+'" target="right">银行卡管理</a>','cardList');
...
/*	if(permissions.xg){
	...
	html.push('<th style="width:8%;">主播名称</th>');
	...
	if(null != data && data.content.length > 0)
	{
			for (var i = 0; i < data.content.length; i++) {
				...
				/*if(permissions.xg){
					html.push('<td>');
						html.push('<a href="javascript:viod(); " data-type="ajax"   data-url="businessman/seller/bindCardInit/updateCardInit.jhtml?sellerid='+$("#sellerid").val()+'&id='+ data.content[i].id+'&sellername='+$("#sellername").val()+'" data-toggle="modal" >修改</a>&nbsp;&nbsp;');
					html.push('</td>');
				}*/
				var nickname = (undefined == $("#nickname").val() ? "-" : $("#nickname").val());
				html.push('<td title ="'+nickname+'">' + substr(nickname,8) + '</td>');
