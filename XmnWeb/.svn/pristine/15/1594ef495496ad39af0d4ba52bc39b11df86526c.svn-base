...
$(document).ready(function() {
	...
	inserTitle(' > 基础数据管理 > <a href="common/area/init.jhtml" target="right">区域管理</a>','userSpan',true);
	notOpenAreaList = $('#notOpenAreaList').page({
		url : 'common/area/lock/init/list.jhtml',
		success : successLock,
		pageBtnNum : 10,
	...
	//区域搜索中查询
	var ld = $("#areaLdId1").areaLd({
		isChosen : true,
		showConfig : [{name:"pareaId",tipTitle:"--省--"},{name:"careaId",tipTitle:"--市--"},{name:"aareaId",tipTitle:"--区--"}]
//		showConfig : [{name:"careaId",tipTitle:"--市--"},{name:"aareaId",tipTitle:"--区--"}]
	});
	//重置选择查询条件
	$("input[data-bus=reset]").click(function(){		
		$("#areaLdId1").find("select").trigger("chosen:updated");
	});
	...
	for (var i = 0; i < data.content.length; i++) {
		...
		if(!b){
			...
			if(permissions.xg == 'true'){
				html.push('<a href="javascript:void(); " data-type="ajax"   data-url="common/area/update/init.jhtml?areaId='+data.content[i].aareaId+'"  data-toggle="modal" >修改</a>');
			}
			if(permissions.open == 'true'){ //开通
				html.push('&nbsp;&nbsp;<a href="javascript:open('+data.content[i].careaId+', '+0+')">开通</a>');
			}
			html.push('</td>');
		}
...
function formAjaxOpen(areaId, status){
	$.ajax({
		type : 'post',
		url : 'common/area/updateStatus.jhtml' + '?t=' + Math.random(),
		data : {
			'areaId':areaId,
			'status':status
		},
		...
		success : function(data) {
			...
			try {
				if (notOpenAreaList != undefined) {
					notOpenAreaList.reload();
				}
			} catch (err) {
				console.log(err);
			}
