...
$(document).ready(function() {
	...
	pageDiv = $('#splitShopDiv').page({
		url : 'businessman/sellerPending/init/listSplitShop.jhtml',
		success : success,
		pageBtnNum : 10,
	...
	inserTitle(' ><a href="businessman/sellerPending/init/splitShop.jhtml?sellerid='+sellerid+'" target="right">总店列表</a>','splitShopList',false);
});
			...
			for (var i = 0; i < data.content.length; i++) {
				...
				if(!flag){
					...
					if(permissions.update=='true'){
						html.push('<a href="businessman/sellerPending/update/init.jhtml?sellerid='+data.content[i].sellerid+'">修改</a>');
					}
					
					//查看权限
					if(permissions.view=='true'){
						html.push('&nbsp;&nbsp;<a href="businessman/sellerPending/getInit.jhtml?sellerid='+data.content[i].sellerid+'&editType=2">查看</a>');
					}
					html.push('</td>');
				...
				if(data.content[i].status == 3)
				{
					//账号
					if(permissions.account=='true'){
						html.push('<td class="text-ellipsis"><a href="businessman/sellerPendingAccount/init.jhtml?sellerid='+data.content[i].sellerid+'">查看</a></td>');
					}
					
					//折扣
					if(permissions.agio=='true'){
						html.push('<td class="text-ellipsis"><a href="businessman/sellerPendingAgio/init.jhtml?sellerid='+data.content[i].sellerid+'">查看</a></td>');
					}
				}else
				{
					//账号
				...
				{
					html.push('<td class="text-ellipsis">审核中</td>');	//状态
				}
				else if(data.content[i].status == 2)
				{
					html.push('<td class="text-ellipsis">未通过</td>');	//状态
				}
				else if(data.content[i].status == 3)
				{
					html.push('<td class="text-ellipsis">已签约</td>');	//状态
				}
				else if(data.content[i].status == 4)
				{
					html.push('<td class="text-ellipsis">未签约</td>');	//状态
				}
				else{
					html.push('<td class="text-ellipsis">未验证</td>');	//状态
				}
