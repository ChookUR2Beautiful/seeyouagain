...
$(document).ready(function() {
	vipCardList = $('#cardholderList').page({
		url : reqUrl+'cardholderList.jhtml',
		success : success,
		pageBtnNum :10,
		pageSize:15,
	...
	inserTitle(' > 商户会员卡管理 > <a href="businessman/vipCard/cardholderListView.jhtml" target="right">会员卡列表</a>','allbillSpan',true);
...
function lockOrNot(islock,noId){
	if(islock!=undefined && noId!=undefined){
		...
		var url = reqUrl+"cardholderList/lock.jhtml";
		$.ajax({
			type : 'post',
			url : url + "?t="
					+ Math.random(),
