...
public class FastFdsFile {
	...
	public Map<String,String> upload(File multipartFile,boolean sltflag) throws IOException, MyException {
		byte[] content = FileUpOrDownLoadUtil.getContent(multipartFile);
		Map<String,String> urlMap=new HashMap<String,String>();
		if (null == multipartFile) {
			return null;
		}
		...
		String result = storageClient.upload_appender_file1(content, originalFilename.substring(originalFilename.lastIndexOf(".") + 1, originalFilename.length()), null) ;
		urlMap.put("url", result);
		if(sltflag){//生成缩略图
			result=PropertiesUtil.readValue("file.upload.fastDFS.http")+result.split("\\.")[0]+"."+result.split("\\.")[1];
			result = storageClient.upload_appender_file1(getCutPic(result,0,0), originalFilename.substring(originalFilename.lastIndexOf(".") + 1, originalFilename.length()), null) ;
			urlMap.put("breviary", result);	
		}
		if (null != storageServer) {
			storageServer.close();
