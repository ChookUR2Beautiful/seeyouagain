...
function updateIsattend(sellerMaketingId,isattend,sellerid) {
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'marketingManagement/activityManagement/discount/initSellerRelateNum/discountUpdateSellerMarketingIsattend.jhtml' + '?t=' + Math.random(),
...
function exitActivity(sellerMaketingId,sellerid) {
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'marketingManagement/activityManagement/discount/initSellerRelateNum/discountExitActivity.jhtml' + '?t=' + Math.random(),
			data : 'id=' +sellerMaketingId+'&sellerid='+sellerid+'&aid='+$('#aid').val(),
			dataType : 'json',
			beforeSend : function(XMLHttpRequest) {
				$('#prompt').show();
			},
