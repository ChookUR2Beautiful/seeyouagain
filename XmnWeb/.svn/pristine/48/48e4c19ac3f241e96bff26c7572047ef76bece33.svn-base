...
<table  class="table table-hover table-bordered table-striped info"  id="memberTable" data-page="${memberPage}" data-total="${memberTotal}">
					...
					<thead><tr >
					<c:if test="${btnAu['memberblactlist/memberList/ordinary/details']||btnAu['memberblactlist/memberList/xunmike/update']}">
					<th >操作</th>
					</c:if>
					<th >用户编号</th>
					<th >用户昵称</th>
					...
					<tbody>
						...
						<c:forEach var="list" items="${member}">
							<tr >
									<td>
									 <c:if test="${btnAu['memberblactlist/memberList/ordinary/details']}">
										<a  href="javascript:viod();" title='详情' data-type='ajax' data-url='memberblactlist/memberList/ordinary/details.jhtml?uid=${list.uid}' data-toggle='modal'>&nbsp;查看</a>	
										 </c:if>
										 <c:if test="${btnAu['memberblactlist/memberList/ordinary/update']}">
										<c:if test="${list.status==4}"><a href="javascript:function viod();"  onclick='upstatus(${list.uid},"1","ordinary")'>&nbsp;恢复到列表</a></c:if>						
									    </c:if>
									</td>
								<td>${list.uid }</td>
								<td>${list.nname }</td>
