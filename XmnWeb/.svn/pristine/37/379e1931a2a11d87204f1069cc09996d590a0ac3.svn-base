...
//			handlePermission : ["sj","xj","tj"],//需要用到checkbox
		...
		cols:[{
			...
			width : 100,
			customMethod : function(value, data) {
				var curPeriodExcitation = (data.periodExcitation == null || data.periodExcitation == 0) ? '-' : data.periodExcitation - (data.curPeriodExcitation == null ? 0 : data.curPeriodExcitation);
