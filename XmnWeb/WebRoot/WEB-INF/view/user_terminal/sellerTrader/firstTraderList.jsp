...
<html>
  ...
  <body style="overflow-x: auto;overflow-y: auto;min-width: 1024px;background:#EEE" class="doc-views with-navbar">
    ...
    <div class="panel">
    	<!-- 城市信息展示 -->
		<div class="panel-body">
			<div id="firstTraderList"></div>
		</div>
	</div>
	<script type="text/json" id="permissions">{firstTraderMgt :'${btnAu['user_terminal/sellerTrader/twoTrader/init'] }'}</script>
   ...
   <script type="text/javascript">
		...
		$(function(){ 
			...
			firstTraderList = $('#firstTraderList').page({
				url : baseURI+"/user_terminal/sellerTrader/init/list.jhtml",
			...
			inserTitle(' > 用户端管理 > <a href="user_terminal/sellerTrader/init.jhtml" target="right">商家分类管理</a>','sellerTrader',true);
		});
		
		function success(data, obj){
			var captionInfo = '<caption style="background:#EED8D8; text-align:left; color:#703636; font-size:16px; line-height:40px;">&nbsp;&nbsp;一级分类&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font>共计【'+data.total+'】个分类&nbsp;</font></caption>';
			obj.find('div').eq(0).scrollTablel({
				...
				handleCols : {
					title : "操作",// 标题
					queryPermission : ["firstTraderMgt"],// 不需要选择checkbox处理的权限
					width : 150,// 宽度
					cols : [{
						title : "管理",// 标题
						linkInfoName : "href",
						linkInfo : {
							href :"user_terminal/sellerTrader/twoTrader/init.jhtml",
							param : ["tid"],
							permission : "firstTraderMgt"
						},
						customMethod : function(value, data) {
							return data.number==0 ? "-": value;
						}
					}] 
				}},permissions);
		}
