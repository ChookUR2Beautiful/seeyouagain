...
<body>
	...
	<div id="cooperatorList"></div>
	<script type="text/javascript">
		$(document).ready(function() {
		var cooperatorList; 
		cooperatorList = $('#cooperatorList').page({
				url : 'business_area/businessArea/init/getcooperatorInfo/init/list.jhtml',
				success : success,
				pageBtnNum : 7,
	...
	function success(data, obj) {
		...
		html.push('<th style="width:50%;">合作商</th>');
		html.push('</tr>');
		html.push('</thead>');
		html.push('<tbody');
		...
		for (var i = 0; i < data.content.length; i++) {		
			...
			html.push('<td>' + (undefined == data.content[i].cooperator ? "-" : data.content[i].cooperator) + '</td>');
			html.push('</tr>');
		}
