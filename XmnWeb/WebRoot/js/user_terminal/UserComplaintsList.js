var userComplaintsList;
var imgRoot = $("#fastfdsHttp").val();

$(document).ready(function() {
	
	userComplaintsList = $('#userComplaintsList').page({
		url : 'user_terminal/userComplaints/init/list.jhtml',
		success : success,
		pageBtnNum : 10,
	...
	inserTitle(' > 用户端管理 > <a href="user_terminal/userComplaints/init.jhtml" target="right">用户投诉</a>','userSpan',true);
...
function success(data, obj) {
	var formId = "searchForm",title = "用户投诉列表",subtitle="个用户投诉";
	...
	obj.find('div').eq(0).scrollTablel({
		...
		handleCols : {
			...
			cols : [{
				...
				linkInfo : {
					modal : {
						url : "user_terminal/userComplaints/update/init.jhtml",// url
						position : "",// 模态框显示位置
						width : "30%", // 模态框宽度
						title : "用户投诉处理" //模态框标题
					},
