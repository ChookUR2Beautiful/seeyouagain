...
$(document).ready(function() {
	...
	inserTitle(' > 营销活动管理 > <a href="fresh/activity/list/init.jhtml" target="right">所有活动</a>', 'activityList', true);
	...
	obj.find('div').eq(0).scrollTablel({
		...
		cols : [ {
			...
			customMethod : function(value, data) {
				...
				else if (new Date().getTime() - new Date(data.endDate).getTime() > 0) {
					return "已结束";
				}
				else if (new Date(data.beginDate).getTime() - new Date().getTime() > 0) {
					return "未开始";
				}
				else {
					return "进行中";
		...
		handleCols : {
			...
			cols : [ {
				...
				linkInfoName : "href",
				linkInfo : {
					href : "fresh/activity/edit/init.jhtml", // url
					param : [ "id" ], // 参数
					permission : "edit" // 列权限
				...
				customMethod : function(value, data) {
					if (data.proceedStatus == 1) {
						return "<a href='fresh/activity/edit/init.jhtml?id=" + data.id + "'>编辑</a>";
					}
					else {
						return '';
					}
			...
			},
				{
					...
					linkInfo : {
						modal : {
							url : "fresh/activity/preview.jhtml", // url
							position : "60px", // 模态框显示位置
							width : "800px"
						},
						param : [ "id" ], // 参数
						permission : "preview" // 列权限
					}
				}, {
					title : "复制链接", // 标题
					linkInfoName : "href",
					linkInfo : {
						modal : {
							url : "fresh/activity/copyurl.jhtml", // url
							position : "60px", // 模态框显示位置
							width : "800px"
						},
						param : [ "id" ], // 参数
						permission : "copyurl" // 列权限
					},
					customMethod : function(value, data) {
						return "<a href='javascript:;' onclick='copy_fun(" + data.id + ")'>复制链接</a>"
					}
				}, {
					title : "立即结束", // 标题
					linkInfoName : "href",
					linkInfo : {
						modal : {
							url : "fresh/activity/end.jhtml", // url
							position : "60px", // 模态框显示位置
							width : "800px"
						},
						param : [ "id" ], // 参数
					...
					linkInfo : {
						modal : {
							url : "fresh/activity/delete.jhtml", // url
							position : "60px", // 模态框显示位置
							width : "800px"
						},
						param : [ "id" ], // 参数
...
function delete_fun(id) {
	showSmConfirmWindow(function() {
		$.post("fresh/activity/delete.jhtml", {
			"id" : id
