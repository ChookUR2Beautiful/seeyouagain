...
function successScroll(data, obj) {
	var captionInfo = '<caption style="background:#EED8D8; text-align:left; color:#703636; font-size:16px; line-height:40px;">&nbsp;&nbsp;活动参与商家列表&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font>共计【'
			+ data.total + '】个参与商家&nbsp;</font></caption>';
	var callbackParam = "isBackButton=true&callbackParam="
			+ getFormParam($("#sellerFromId").serialize());
	...
	obj.find('div').eq(0).scrollTablel({
		...
		handleCols : {
			title : "操作",// 标题
			queryPermission : [ "exitActivity", "ck" ],// 不需要选择checkbox处理的权限
			// width : 240,// 宽度
			// 当前列的中元素
			cols : [ {
				title : "退出活动",// 标题
				...
				customMethod : function(value, data) {
					...
					if (data.status != undefined
							&& data.status == 3) {
						result = '<a href="javascript:resetPassW('
								+ data.sellerid
								+ ', '
								+ 2
								+ ');">重置提现密码</a>';
...
function success(data, obj) {
	...
	html.push('<th>折扣类型</th>');
	html.push('<th>商家折扣</th>');
	html.push('<th>用户分账</th>');	
	html.push('<th>佣金补贴</th>');
...
*/				html.push('<td>' + (undefined == data.content[i].phoneid ? "-" : data.content[i].phoneid) + '</td>');
				...
				//折扣类型
				var agioTypeTemp = "-";
				if(data.content[i].agioType == 1){
					agioTypeTemp = "常规折扣";
				}else if(data.content[i].agioType == 2){
					agioTypeTemp = "每周折扣";
				}else if(data.content[i].agioType == 3){
					agioTypeTemp = "特殊折扣";
				}
				html.push('<td class="text-ellipsis">'+agioTypeTemp+'</td>');
				//商家折扣
				...
				html.push('<td class="text-ellipsis" name="status" initValue="'+data.content[i].Strisonline+'">'+Strisonline+'</td>');	//上线状态
				//
				var isattend = "-";
				if(data.content[i].isattend  == 0)
				{
				...
				html.push('<td class="text-ellipsis" name="status" initValue="'+data.content[i].isattend+'">'+isattend+'</td>');	//参与状态
				html.push('</tr>');
			}
	}
	else
...
function updateIsattend(sellerMaketingId,isattend,sellerid) {
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'marketingManagement/activityManagement/commission/updateSellerMarketingIsAttend.jhtml' + '?t=' + Math.random(),
...
function exitActivity(sellerMaketingId,sellerid) {
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'marketingManagement/activityManagement/commission/commissionExitActivity.jhtml' + '?t=' + Math.random(),
