...
$(document).ready(function() {
	//满赠活动
	manzengjfList = $('#manzengjf').page({
		url : 'marketingManagement/activityManagement/manzengjf/init/list.jhtml',
		success : manzengjfSuccess,
		pageBtnNum : 10,
		paramForm : 'manzengjfForm',
		param:{
			type:"7"
		}
	});
	limitedDate({form:"#manzengjfForm",startDateName:"sDateBegin",endDateName:"eDateEnd",format:'yyyy-mm-dd hh:ii'});
	
});

function manzengjfSuccess(data, obj) {
	var html = [];
	html.push('<table class="table table-hover table-bordered table-striped info" >');
	html.push('<caption style="background:#EED8D8; text-align:center; color:#703636; font-size:16px; line-height:40px;">满赠积分活动列表</caption>');
	html.push('<thead>');
	html.push('<tr>');
	...
	if(null != data && data.content.length > 0)
	{
		...
		if(b){
			...
			if(permissions.mzupdate=='true'){
				...
				html.push('<a href="javascript:viod(); " data-type="ajax"  data-width="60%" data-url="marketingManagement/activityManagement/manzeng/update/init.jhtml?type=7&aid='+data.content[i].aid +'"  data-toggle="modal" >修改</a>&nbsp;&nbsp;');
		    }
			if(permissions.mzcheck=='true'){
				
				html.push('<a href="javascript:viod(); " data-type="ajax" data-width="50%" data-url="marketingManagement/activityManagement/manzeng/check/init.jhtml?type=7&aid='+data.content[i].aid +'"  data-toggle="modal" >查看</a>&nbsp;&nbsp;');
			}
			html.push('</td>');
		}
			var callbackParam="&isBackButton=true&callbackParam="+getFormParam($("#manzengjfForm").serialize());
 ...
 * 批量设置平台补贴折扣
 */
function mzjfActivityManager(){
		var aids= new Array();
		var aids=manzengjfList.getIds().split(",");
		if(!manzengjfList.getIds()){
