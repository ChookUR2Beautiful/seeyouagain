		...
		var callbackParam="&isBackButton=true&callbackParam="+getFormParam($("#sellerPendingFromId").serialize());
			for (var i = 0; i < data.content.length; i++) {
				...
				if(permissions.exitActivity){
				  ...
				  html.push("<a href='javascript:#'  onclick='exitActivity("+data.content[i].sellerMaketingId+','+data.content[i].sellerid+")'>退出活动</a>");
				 }
				  html.push('</td>');
				}
...
function exitActivity(sellerMaketingId,sellerid){
	showSmConfirmWindow(function() {
		$.ajax({
			type:'post',
			url :'marketingManagement/activityManagement/scratchCard/initSellerRelateNum/scratchCardExitActivity.jhtml' + '?t=' + Math.random(),
