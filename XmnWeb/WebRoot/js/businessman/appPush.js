...
$(document).ready(function() {
	appPushList = $('#appPushList').page({
		url : 'businessman/appPush/init/list.jhtml',
		success : success,
		pageBtnNum : 10,
	...
	inserTitle(' > 商家管理 > <a href="businessman/appPush/init.jhtml" target="right">营销信息推送</a>','userSpan',true);

	$("#export").click(function(){
		$form = $("#searchForm").attr("action","businessman/appPush/export.jhtml");
		$form[0].submit();
	});
});
		...
		for (var i = 0; i < data.content.length; i++) {
			...
			if(!Ispermissions){
				...
				if(permissions.update=='true'){
					html.push('<a href="javascript:viod(); " data-type="ajax"   data-url="businessman/appPush/update/init.jhtml?tid='+data.content[i].tid+'"  data-toggle="modal" >查看</a>&nbsp;&nbsp;');
				}
			...
			var object = (data.content[i].object ? data.content[i].object : "全部");
			html.push('<td title ="'+object+'">' + substr(object) + '</td>');
			...
			html.push('<td title ="'+content+'">' + substr(content) + '</td>');
...
function remove(tid) {
	...
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'businessman/appPush/delete.jhtml' + '?t=' + Math.random(),
			data : 'tid=' + tid,
			dataType : 'json',
