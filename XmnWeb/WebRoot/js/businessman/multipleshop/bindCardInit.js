...
$(document).ready(function() {
	cardList = $('#cardList').page({
		url : 'businessman/multipShop/bindCardList.jhtml',
		success : success,
		pageBtnNum : 10,
...
function success(data, obj) {
	...
	if(null != data && data.content.length > 0)
	{
			for (var i = 0; i < data.content.length; i++) {
				...
				if(permissions.xg){
					html.push('<td>');
						html.push('<a href="javascript:viod(); " data-type="ajax"   data-url="businessman/multipShop/bindCardInit/updateCardInit.jhtml?sellerid='+$("#sellerid").val()+'&id='+ data.content[i].id+'&sellername='+$("#sellername").val()+'" data-toggle="modal" >修改</a>&nbsp;&nbsp;');
					html.push('</td>');
				}
