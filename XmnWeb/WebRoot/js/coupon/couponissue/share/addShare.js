...
$(function() {
	inserTitle(
			' >优惠券管理 > <span><a href="coupon/couponissue/init.jhtml.jhtml" target="right">优惠券发放管理</a> >添加分享发放',
			'couponList', true);
	initConfig();
	initView();
	...
	/*
	 * , messages : { amount : { required : "必填!", number : "必须是数字！" }, maxTimes : {
	 * required : "必填!", digits : "必须是整数！", min : "最小值为1" }, dateStart : {
	 * required : "必填！" }, dateEnd : { required : "必填！", } }
	 */
	}, formAjax)
...
function formAjax() {
	var url = "coupon/couponissue/share/add.jhtml"
	var data = jsonFromt($('#addShareForm').serializeArray());
	// 更新area
	if (data) {
...
function customValidate() {
	...
	var IsAreaSuccess = function() {
		...
		return checkSelect('#addShareForm', "#areaInfo", selectAray, true);
	}();
	//
...
function initConfig() {
	/*
	 * config.shareCoupon = '<div class="input-group" data-num="1">' +
	 * $("#shareCoupons").find(".input-group").html() + '<div>';
	 * config.orderCoupon = '<div class="input-group" data-num="0">' +
