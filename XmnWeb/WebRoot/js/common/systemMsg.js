...
$(document).ready(function() {
	systemMsgList = $('#systemMsgList').page({
		url : 'common/systemMsg/init/list.jhtml',
		success : success,
		pageBtnNum : 10,
	...
	inserTitle(' > 合作商管理 > <a href="common/systemMsg/init.jhtml" target="right">推送消息管理</a>','userSpan',true);

	$("#export").click(function(){
		$form = $("#searchForm").attr("action","common/systemMsg/export.jhtml");
		$form[0].submit();
	});
...
function success(data, obj) {
	...
	for (var i = 0; i < data.content.length; i++) {
		...
		html.push('<td><a href="javascript:viod(); " data-type="ajax"   data-url="common/systemMsg/update/init.jhtml?sid='+data.content[i].sid+'"  data-toggle="modal" >修改</a>&nbsp;/&nbsp;<a href="javascript:remove('+data.content[i].sid+')">删除</a></td>');
...
function remove(sid) {
	showSmConfirmWindow(function() {
		$.ajax({
			type : 'post',
			url : 'common/systemMsg/delete.jhtml' + '?t=' + Math.random(),
			data : 'sid=' + sid,
			dataType : 'json',
