$.fn.searchChosen = function(param){
	...
	var showChose = function(value, show){
		//console.info(value);
		$(document.getElementById('iframetriggerModal').contentWindow.document).find(".search-choice").remove();
...
//				$this.val($this.val() + value + opts.separator);
				...
				//添加商家服务员配置时将sellerid填写到sellerId文本框
				$("#sellerId").val(value);
				$("#sellername").val(show);
				//添加商家服务员推广信息时将sellerid填写到sellerid文本框
				$("#sellerid").val(value);
				//添加商家服务员管理时查询商家服务员列表
				$.ajax({
					url : "businessman/sellerSubsidy/spread/add/SelectWaiterList.jhtml?sellerid="+value,
					dataType:"json",
					method : "get", 
					success:function(data){
						$("#phoneid").empty().append('<option value="">请选择服务员账号</option>').attr("readonly", false);
						for(var i=0;i<data.length;i++){
						$("#phoneid").append('<option value="'+data[i].account+','+data[i].accountid+','+data[i].sellername+'">'+((data[i].fullname!=null||data[i].remarks!=null)?'(':"")+(data[i].fullname==null?"":data[i].fullname+'-')+(data[i].remarks==null?"":data[i].remarks)+''+((data[i].fullname!=null||data[i].remarks!=null)?')':"")+data[i].account+'</option>');
						}
					}
				});
			 }
