...
public class TLiveGivedGiftService extends BaseService<TLiveGivedGift> {
	...
	/**
	 * 方法描述：将手机号转化为liverId(观看直播的用户ID)查询条件 <br/>
	 * 创建人：  huang'tao <br/>
	 * 创建时间：2017-6-19上午11:57:25 <br/>
	 * @param liveGivedGift
	 */
	public void convertPhoneParam(TLiveGivedGift liveGivedGift) {
		// TODO Auto-generated method stub
		String phone = liveGivedGift.getPhone();
		if(StringUtils.isNotBlank(phone)){
			BLiver liver=new BLiver();
			liver.setPhone(phone);
			BLiver liverInfo = liverDao.selectBLiver(liver);
			Integer liverId = liverInfo.getId();
			liveGivedGift.setLiverId(liverId);
		}
	}
