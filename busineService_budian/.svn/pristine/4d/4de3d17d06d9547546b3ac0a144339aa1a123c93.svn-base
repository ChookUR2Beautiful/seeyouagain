 ...
 */
@Service("generalRechargeLiveOrder")
public class GeneralRechargeLiveOrder implements RechargeLiveOrderService<LivePayOrder> {

	// 初始化日志类
	private final Logger log = Logger.getLogger(GeneralRechargeLiveOrder.class);
	...
	public LivePayOrder initRechargeOrderLedger(LivePayOrder order) {
		//log.info("常规充值，没有相应奖励");
		initRechargeOrderLedgerInfo(order,null);
		liveLedgerService.countUserEnergyNum(order.getUid(), order.getPayment(),order.getOrderNo(),0);
		return null;
	}
	...
	public void initRechargeOrderLedgerInfo(LivePayOrder payOrder,LiveFansRankDetail liveFansRankDetail){
		...
		livePrivilege.setObjectOriented(0);
