 ...
 */
@Service("vipRechargeLiveOrder")
public class VipRechargeLiveOrder implements RechargeLiveOrderService<LivePayOrder> {

	// 初始化日志类
	private final Logger log = Logger.getLogger(VipRechargeLiveOrder.class);
...
//			}
			...
			if(ledgeringOrder==null || ledgeringOrder.getPayment().compareTo(order.getPayment())<0){
				liveLedgerService.updateUrsRankInfo((long)order.getUid(), liveFansRank.getId(), 1);
			}
