 ...
 */
@Service("liverDividends0610Quertz")
public class LiverDividends0610QuertzService {
    /*
     * 日志记录
     */
    private final Logger log = Logger.getLogger(LiverDividends0610QuertzService.class);
	...
	public void liverDividends(){
		...
		try{
			 ...
			 *   }
			 */
			getLock = redisLockUtil.getLock("busineService:quartz:"+getClass().getName(),0);
			if(getLock){
					...
					for(Integer uid:ursList){
						try{
							...
							for(LivePrivilege order: orderList){
								...
								/* 会员实际可领现金红包金额， */
								BigDecimal realOrderCash = order.getBaseAmount()==null?orderCash:order.getBaseAmount();
								BigDecimal unLedgerTemp = BigDecimal.ZERO;	//解决当调整红包现金鸟币分配比例时，用户已领红包超出调整后可领现金上额时，导致实际用户可领红包现金+鸟币>payment的情况
								BigDecimal unLedgerCash = realOrderCash.subtract(order.getCurrentDividendLedger());
			...
			if(getLock){
				redisLockUtil.releaseLock("busineService:quartz:"+getClass().getName());
			}
		}
	}
